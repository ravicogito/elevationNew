...
class Mailchimpmodel extends CI_Model {
	...
	{
		    $sql = "SELECT * FROM el_customer WHERE customer_ref_id=$rfID";
            $rs = $this->db->query($sql);
	...
	function addMailchimp($data) {
		...
		if(!empty($data)){
			...
			$this->db->insert('el_mailchimp_template', $data);
	...
	function allmailchimpList() {
		$this->db->select('*');

        $this->db->from('el_mailchimp_template');
		
		$this->db->order_by("el_mailchimp_template.id", "desc");
	...
	function editMailchimpdata($data = '',$id ='') {
		
		if(!empty($data)){
			
			 $this->db->where('id', $id);

             $this->db->update('el_mailchimp_template', $data);
	...
	function publishMailchimpdata($data = '',$id ='') {
	...
	function checkpublishMailchimpdata() {
		$this->db->select('*');
	...
	function deleteMailchimpdata($id ='') {
		
		if(!empty($id)){
			
			 $this->db->where('id', $id);

             $this->db->delete('el_mailchimp_template');
