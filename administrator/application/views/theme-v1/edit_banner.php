    ...
    <section class="content">
      ...
      <?php } ?>
		...
		<input type="hidden" name="banner_id" value="<?php echo $banner_edit['banner_id']; ?>">
			<div class="event_field">				<div class="form-group">					<label>Select Page</label>					<select name="banner_type" id="banner_type" class="form-control">						<option value="">Select</option>							<?php if(!empty($page_list)){							foreach($page_list as $list){						?>								<option value="<?php echo $list['page_id']?>" <?php echo($banner_edit['banner_type'] == $list['page_id'] ? 'selected' : ''); ?>><?php echo $list['page_title']?></option>												<?php } } ?>											</select>				</div>				<div class="form-group">				  <label>Title</label>				  <input type="text" class="form-control" id="ptitle" name="ptitle" value="<?php if(!empty($banner_edit['banner_title'])){ echo  $banner_edit['banner_title']; } ?>">				</div><BR>								<div class="box box-info">				<div class="box-header">                <h3 class="box-title">Page Description                </h3>                <!-- tools box -->                <div class="pull-right box-tools">                  <button type="button" class="btn btn-info btn-sm" data-widget="collapse" data-toggle="tooltip"                          title="Collapse">                    <i class="fa fa-minus"></i></button>                  <button type="button" class="btn btn-info btn-sm" data-widget="remove" data-toggle="tooltip"                          title="Remove">                    <i class="fa fa-times"></i></button>                </div>                <!-- /. tools -->              </div>              <!-- /.box-header -->              <div class="box-body pad">				<div class="box-body pad" id="editorContainer">					<textarea id="editor1" name="editor1" rows="10" cols="80"><?php if(!empty($banner_edit['banner_content'])){ echo  $banner_edit['banner_content']; } ?></textarea>				</div>              </div>            </div>
				<div class="form-group">
