...
<?php }
else if($_SESSION['cartVal']['size']=='package'){ ?>
<p>This must be the same boat and different images! - Customer can add additional 4x6's of different images for<span>$7/each</span></p>
<?php }?>
</div>
<br class="clear" />
	...
	<?php 
		$price    	       				    = $_SESSION['cartVal']['size_price'];
		$size     	      					= $_SESSION['cartVal']['size'];
		$images					   		    = $_SESSION['cartVal']['images'];
		$images_id 							= $_SESSION['cartVal']['imagesID'];
		$all_images 						= explode('~',$images);
		$all_images_id 						= explode('-',$images_id);
		$quantity  							= $_SESSION['cartVal']['quantity'];
...
<a href="javascript:void(0);" id="rmv_<?=$key?>" class="remove remove_order"><img src="<?=base_url()?>assets/images/qty_deleteicon.png" class="delete"></a>
	...
	<p class="price"><?php echo '$'.number_format($price,2); ?></div></p>
    <br class="clear" />
	</div>
