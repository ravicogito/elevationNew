...
var mediaNameArr		  = new Array();
  ...
  $(document).ready(function() {
	...
	$.ajax({
				type:"POST",
				url:"<?php echo base_url(); ?>Favourite/cartNew",
				dataType: "json",
				data:
				{
					formdata:html,
				...
				{
				 //alert(response.process);	
				  window.location.href = "<?php echo base_url(); ?>CartAdd";
	...
	$.ajax({
				...
				url:"<?php echo base_url(); ?>Favourite/cartNewDigital",
				dataType: "json",
				data:
				{
					formdata:html,
  ...
  function digitalProcsNext(){
		...
		$.ajax({
				...
				success:function(response)
				{
					...
					if(response['download'] =='no'){	
					  window.location.href = "<?php echo base_url(); ?>CartAdd";
					}
					else{
						
						window.location.href = "<?php echo base_url(); ?>CartAdd/placeOrder";
					}
...
function openapackage(evt, packageName) {
	/*if(packageName == 'Print'){
		...
		$(".tab_section .tab").children("button:eq(1)").prop("disabled",true);
	}
	else{
		$(".tab_section .tab").children("button:eq(0)").prop("disabled",true);
	}*/
...
<div id="loadingGIF">
<img src="<?php echo base_url()?>assets/images/ajax-loader.gif" alt=""/><p style="font-size:28"><strong>PROCESSING, PLEASE WAIT...</strong></p>
</div>
<div class="contwrap">
...
<input type="hidden" name="totalevntimg" class="totalevntimg" value="">
</div>
</div>
<br style="clear: both;">
